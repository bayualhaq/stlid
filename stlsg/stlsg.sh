...
#stlsg (Wegare)
clear
udp2="$(cat /root/akun/stlsg.txt | grep -i udp | cut -d= -f2)" 
user2="$(cat /root/akun/stlsg.txt | grep -i user | cut -d= -f2 | head -n1)" 
host2="$(cat /root/akun/stlsg.txt | grep -i host | cut -d= -f2 | head -n1)"
port2="$(cat /root/akun/stlsg.txt | grep -i port | cut -d= -f2 | head -n1)" 
bug2="$(cat /root/akun/stlsg.txt | grep -i bug | cut -d= -f2)" 
pass2="$(cat /root/akun/stlsg.txt | grep -i pass | cut -d= -f2)" 
pp2="$(cat /root/akun/stlsg.txt | grep -i pp | cut -d= -f2 | tail -n1)" 
payload2="$(cat /root/akun/stlsg.txt | grep -i payload | cut -d= -f2)" 
proxy2="$(cat /root/akun/stlsg.txt | grep -i proxy | cut -d= -f2)" 
met2="$(cat /root/akun/stlsg.txt | grep -i met | cut -d= -f2 | head -n1)" 
...
server.close()" > /usr/bin/http-stlsg
chmod +x /usr/bin/http-stlsg
elif [ "$met" = "https" ]; then
echo "Masukkan bug" 
...
clear
met="$(cat /root/akun/stlsg.txt | grep -i met | cut -d= -f2)" 
if [ "$met" = "http" ]; then
user="$(cat /root/akun/stlsg.txt | grep -i user | cut -d= -f2)" 
host="$(cat /root/akun/stlsg.txt | grep -i host | cut -d= -f2 | head -n1)"
port="$(cat /root/akun/stlsg.txt | grep -i port | cut -d= -f2 | head -n1)" 
pass="$(cat /root/akun/stlsg.txt | grep -i pass | cut -d= -f2)" 
http-stlsg > /dev/null &
sleep 2
...
EOF
chmod +x /usr/bin/ping-stlsg
/usr/bin/ping-stlsg > /dev/null 2>&1 &
elif [ "${tools}" = "3" ]; then
host="$(cat /root/akun/stlsg.txt | grep -i host | cut -d= -f2 | head -n1)" 
route="$(cat /root/akun/ipmodem.txt | grep -i ipmodem | cut -d= -f2 | tail -n1)" 
#killall screen
killall -q badvpn-tun2socks ssh ping-stlsg stunnel sshpass http-stlsg screen
...
# BEGIN AUTOREKONEKstlsg
*/1 * * * *  autorekonek-stlsg
# END AUTOREKONEKstlsg
EOF
...
sed -i "/^# BEGIN AUTOREKONEKstlsg/,/^# END AUTOREKONEKstlsg/d" /etc/crontabs/root > /dev/null
/etc/init.d/cron restart
