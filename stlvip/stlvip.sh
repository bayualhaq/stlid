...
#stlvip (Wegare)
clear
udp2="$(cat /root/akun/stlvip.txt | grep -i udp | cut -d= -f2)" 
user2="$(cat /root/akun/stlvip.txt | grep -i user | cut -d= -f2 | head -n1)" 
host2="$(cat /root/akun/stlvip.txt | grep -i host | cut -d= -f2 | head -n1)"
port2="$(cat /root/akun/stlvip.txt | grep -i port | cut -d= -f2 | head -n1)" 
bug2="$(cat /root/akun/stlvip.txt | grep -i bug | cut -d= -f2)" 
pass2="$(cat /root/akun/stlvip.txt | grep -i pass | cut -d= -f2)" 
pp2="$(cat /root/akun/stlvip.txt | grep -i pp | cut -d= -f2 | tail -n1)" 
payload2="$(cat /root/akun/stlvip.txt | grep -i payload | cut -d= -f2)" 
proxy2="$(cat /root/akun/stlvip.txt | grep -i proxy | cut -d= -f2)" 
met2="$(cat /root/akun/stlvip.txt | grep -i met | cut -d= -f2 | head -n1)" 
...
server.close()" > /usr/bin/http-stlvip
chmod +x /usr/bin/http-stlvip
elif [ "$met" = "https" ]; then
echo "Masukkan bug" 
...
clear
met="$(cat /root/akun/stlvip.txt | grep -i met | cut -d= -f2)" 
if [ "$met" = "http" ]; then
user="$(cat /root/akun/stlvip.txt | grep -i user | cut -d= -f2)" 
host="$(cat /root/akun/stlvip.txt | grep -i host | cut -d= -f2 | head -n1)"
port="$(cat /root/akun/stlvip.txt | grep -i port | cut -d= -f2 | head -n1)" 
pass="$(cat /root/akun/stlvip.txt | grep -i pass | cut -d= -f2)" 
http-stlvip > /dev/null &
sleep 2
...
EOF
chmod +x /usr/bin/ping-stlvip
/usr/bin/ping-stlvip > /dev/null 2>&1 &
elif [ "${tools}" = "3" ]; then
host="$(cat /root/akun/stlvip.txt | grep -i host | cut -d= -f2 | head -n1)" 
route="$(cat /root/akun/ipmodem.txt | grep -i ipmodem | cut -d= -f2 | tail -n1)" 
#killall screen
killall -q badvpn-tun2socks ssh ping-stlvip stunnel sshpass http-stlvip screen
...
# BEGIN AUTOREKONEKstlvip
*/1 * * * *  autorekonek-stlvip
# END AUTOREKONEKstlvip
EOF
...
sed -i "/^# BEGIN AUTOREKONEKstlvip/,/^# END AUTOREKONEKstlvip/d" /etc/crontabs/root > /dev/null
/etc/init.d/cron restart
